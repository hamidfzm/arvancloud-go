...
func (v *JSONType) UnmarshalJSON(src []byte) error {
	...
	enumTypeValue := JSONType(value)
	for _, existing := range AllowedJSONTypeEnumValues {
	...
	return fmt.Errorf("%+v is not a valid JSONType", value)
}

// NewJSONTypeFromValue returns a pointer to a valid JSONType
...
func NewJSONTypeFromValue(v string) (*JSONType, error) {
	...
	if ev.IsValid() {
		...
		return nil, fmt.Errorf("invalid value '%v' for JSONType: valid values are %v", v, AllowedJSONTypeEnumValues)
	}
}
