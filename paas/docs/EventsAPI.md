...
------------- | ------------- | -------------
[**ListNamespacedEvent**](EventsAPI.md#ListNamespacedEvent) | **Get** /api/v1/namespaces/{namespace}/events | list or watch objects of kind Event
[**ReadNamespacedEvent**](EventsAPI.md#ReadNamespacedEvent) | **Get** /api/v1/namespaces/{namespace}/events/{name} | read the specified Event



## ListNamespacedEvent

> V1EventList ListNamespacedEvent(ctx, namespace).Pretty(pretty).LabelSelector(labelSelector).FieldSelector(fieldSelector).IncludeUninitialized(includeUninitialized).Watch(watch).ResourceVersion(resourceVersion).TimeoutSeconds(timeoutSeconds).Limit(limit).Continue_(continue_).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.EventsAPI.ListNamespacedEvent(context.Background(), namespace).Pretty(pretty).LabelSelector(labelSelector).FieldSelector(fieldSelector).IncludeUninitialized(includeUninitialized).Watch(watch).ResourceVersion(resourceVersion).TimeoutSeconds(timeoutSeconds).Limit(limit).Continue_(continue_).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `EventsAPI.ListNamespacedEvent``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ListNamespacedEvent`: V1EventList
    fmt.Fprintf(os.Stdout, "Response from `EventsAPI.ListNamespacedEvent`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiListNamespacedEventRequest struct via the builder pattern
...
> V1Event ReadNamespacedEvent(ctx, namespace, name).Pretty(pretty).Export(export).Exact(exact).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.EventsAPI.ReadNamespacedEvent(context.Background(), namespace, name).Pretty(pretty).Export(export).Exact(exact).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `EventsAPI.ReadNamespacedEvent``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ReadNamespacedEvent`: V1Event
    fmt.Fprintf(os.Stdout, "Response from `EventsAPI.ReadNamespacedEvent`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiReadNamespacedEventRequest struct via the builder pattern
