...
func (a *CachingAPIService) CachingDeprecatedPurgeExecute(r ApiCachingDeprecatedPurgeRequest) (*MessageResponse, *http.Response, error) {
	...
	if r.purgeUrls != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "purge_urls", s.Index(i).Interface(), "multi")
			}
		} else {
	...
	if r.purgeTags != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "purge_tags", s.Index(i).Interface(), "multi")
			}
		} else {
